...
class GPT(nn.Module):
    ...
    def __init__(self, cfg: dict, embedder: EMREmbedding, use_checkpoint: bool=True):
        ...
        # S (direction C): Time-to-terminal regression head.
        # Predicts log1p(hours-to-terminal) at every position. Provides explicit
        # long-horizon temporal awareness that compensates for the bounded
        # magnitude head's restricted Δt range.
...
# ---------------------------------------------------- forward ---- #
    def forward(self, parent_raw_ids, concept_ids, value_ids, position_ids,
        ...
        # S: magnitude head HARD-BOUNDED to 24h (R's fix). Combined with TTT head
        # below, gives explicit long-horizon awareness compensating for the
        # bounded Δt range.
        ...
        # S: TTT head output
        ttt_pred = self.ttt_head(x).squeeze(-1)        # [B, T] log1p(hours-to-terminal)
    ...
    def run_epoch(loader, epoch, train_flag=False):
        ...
        with torch.set_grad_enabled(train_flag):
            for batch in tqdm(loader, desc="Training" if train_flag else "Validation", leave=False, mininterval=5.0, miniters=10, dynamic_ncols=True):
                ...
                # S: Time-to-terminal regression loss (direction C).
                # Predict log1p(hours-to-terminal) at each query position. Forces
                # the backbone hidden state to encode time-to-terminal explicitly,
                # compensating for the bounded magnitude head's restricted Δt range.
