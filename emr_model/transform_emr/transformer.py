...
class GPT(nn.Module):
    ...
    def __init__(self, cfg: dict, embedder: EMREmbedding, use_checkpoint: bool=True):
        ...
        # N (direction E): narrow terminal tau from 168h to 12h. The wide kernel
        # taught the model "TERMINAL within 168h" at every position, since most
        # sequences end within 168h — driving Phase 2 into the TERMINAL-dominant
        # local minimum (gen_median_steps=4). With tau=12h, TERMINAL soft-label
        # decays from 1.0 to ~0.14 over 24h — only positions within ~24h of
        # actual terminal contribute meaningful BCE gradient.
        ...
        # N (direction E): freeze tau_lm for terminal token IDs so the optimizer
        # cannot drift it back toward 168h. Gradient hook zeros the gradient at
        # terminal indices on every backward pass. Non-terminal taus remain
        # fully learnable.
        if self._terminal_ids.numel() > 0:
