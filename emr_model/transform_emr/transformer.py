...
def finetune_transformer(model, train_dl, val_dl, resume=True,
    ...
    def _make_p3_optimizer(m):
        # P3 — three head-group regime:
        #   outcome_head + bias_proj : full p3_lr (the new task-specific heads)
        #   lm_head (tied to embedder.position_embed): 0.1 * p3_lr — small
        #     enough to keep the LM head adapting alongside bias_proj without
        #     drifting away from the joint P2 optimum. lm_head.weight is the
        #     SAME tensor as embedder.position_embed.weight (weight tying), so
        #     placing it in this group also bumps the input embedding's LR
        #     from 1e-6 (backbone) to 1e-5 — a deliberate side effect: the
        #     input embedding and LM head must adapt together for the
        #     coupling to form.
        #   everything else: backbone LR (p3_lr * backbone_lr_factor).
        lm_w = m.lm_head.weight   # tied with embedder.position_embed.weight
        ...
        backbone_params = [p for _, p in m.named_parameters() if id(p) not in all_head_ids]
