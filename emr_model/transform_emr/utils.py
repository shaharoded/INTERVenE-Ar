...
def get_temporal_multi_hot_targets(
    ...
    # exp59/exp60: optionally use wider future windows for specific subsets of tokens.
    # Single-tier: pass wide_token_ids + wide_window_size.
    # Two-tier (exp60): the legacy single-tier path is wrapped into the same overlay
    # loop. Each tier reuses the same prefix sums — one extra searchsorted + gather
    # per tier. Goal: provide denser positive signal for rare/critical tokens
    # (terminals = 168 h, clinical outcomes = 48 h) so the LM head learns to assign
    # them higher logits at pre-event positions, addressing the inference-termination
    # miss-rate and under-supported complication-token training.
    _wide_specs = []
