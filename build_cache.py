...
api.py tries to save a 40GB EMRDataset to disk as the processed cache, which exceeds
the 46.6GB cgroup memory limit. This script builds a MINIMAL cache:
  - train_ds, val_ds: 1 patient each (just enough for ctx_dim detection)
  - tokenizer: real tokenizer from checkpoints/
  - test_raw: full raw test DataFrames (processed fresh in evaluate_on_test_set)
  - sizes: (n_train, n_val, n_test) = (39954, 8562, 8562)
  - key: matches api.py cache_key = (None, 42, 0.15, 0.15, USE_QA_DATA)

api.py fast-path checks: cache.get("key") == cache_key — this passes.
ctx_dim is read from the first batch of embedder_train_dl (1-patient batch) — correct.
...
print("Done. Run: python api.py --eval-only")
